...
registerSuite({
		...
		'has loader tests': {
			'both feature and no-feature modules provided'() {
				...
				const actualHasBrowser = hasNormalize('host-browser?intern:intern!object', require.toAbsMid);
				assert.strictEqual(actualHasBrowser, expectedHasBrowser);

				const expectedHasNode = has('host-node') ? 'intern/main' : 'intern!object';
				const actualHasNode = hasNormalize('host-node?intern:intern!object', require.toAbsMid);
			...
			'only feature module provided'() {
				...
				const actualHasBrowser = hasNormalize('host-browser?intern', require.toAbsMid);
				assert.strictEqual(actualHasBrowser, expectedHasBrowser);

				const expectedHasNode = has('host-node') ? 'intern/main' : undefined;
				const actualHasNode = hasNormalize('host-node?intern', require.toAbsMid);
				assert.strictEqual(actualHasNode, expectedHasNode);
			},

			'only no-feature module provided'() {
				const expectedHasBrowser = has('host-browser') ? 'intern/main' : null;
				const actualHasBrowser = hasNormalize('host-node?:intern', require.toAbsMid);
				assert.strictEqual(actualHasBrowser, expectedHasBrowser);

				const expectedHasNode = has('host-node') ? 'intern/main' : null;
				const actualHasNode = hasNormalize('host-browser?:intern', require.toAbsMid);
				assert.strictEqual(actualHasNode, expectedHasNode);
			},
			...
			'load test resourceId provided'() {
				...
				hasLoad(resourceId, <RootRequire> <any> stubbedRequire, loadedStub);
